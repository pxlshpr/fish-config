function kimi-song-v7 --description "Run the Chunes Kimi song-tagging prompt (v7: strict Pattern A + artist cross-check + empty-on-no-match). Usage: kimi-song-v7 'Title - Artist' [--model id] [--album 'Album Name']"
    set -l input ""
    set -l album ""
    set -l model ""
    ...
    if test -z "$input"
        echo "usage: kimi-song-v7 'Title - Artist' [--model id] [--album 'Album Name']"
        echo ""
        __kimi_list_models
        return 2
    end
    ...
    __kimi_call_song_v7 "$title" "$artist" "$album" "$model" "$api_key"
end
