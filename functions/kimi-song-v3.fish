function kimi-song-v3 --description "Run the Chunes Kimi song-tagging prompt (v3: genre→vocals hint, 'no-vocals' naming). Shows a model picker if --model not given. Usage: kimi-song-v3 'Title - Artist' [--model id] [--album 'Album Name']"
    set -l input ""
    set -l album ""
    set -l model ""
    ...
    if test -z "$input"
        echo "usage: kimi-song-v3 'Title - Artist' [--model id] [--album 'Album Name']"
    ...
    __kimi_call_song_v3 "$title" "$artist" "$album" "$model" "$api_key"
end
