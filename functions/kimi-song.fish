function kimi-song --description "Run the Chunes Kimi song-tagging prompt. Shows a model picker if --model not given. Usage: kimi-song 'Title - Artist' [--model id] [--album 'Album Name']"
    set -l input ""
    set -l album ""
    set -l model ""
    ...
    if test -z "$input"
        echo "usage: kimi-song 'Title - Artist' [--model id] [--album 'Album Name']"
    ...
    __kimi_call_song "$title" "$artist" "$album" "$model" "$api_key"
end
