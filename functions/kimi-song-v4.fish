function kimi-song-v4 --description "Run the Chunes Kimi song-tagging prompt (v4: artist-first reasoning, Lofi default for obscure artists, downweight DnB/Trap). Usage: kimi-song-v4 'Title - Artist' [--model id] [--album 'Album Name']"
    set -l input ""
    set -l album ""
    set -l model ""
    ...
    if test -z "$input"
        echo "usage: kimi-song-v4 'Title - Artist' [--model id] [--album 'Album Name']"
    ...
    __kimi_call_song_v4 "$title" "$artist" "$album" "$model" "$api_key"
end
