function kimi-song-v2 --description "Run the Chunes Kimi song-tagging prompt (v2: anti-stereotype, stricter vocals). Shows a model picker if --model not given. Usage: kimi-song-v2 'Title - Artist' [--model id] [--album 'Album Name']"
    set -l input ""
    set -l album ""
    set -l model ""
    ...
    if test -z "$input"
        echo "usage: kimi-song-v2 'Title - Artist' [--model id] [--album 'Album Name']"
    ...
    __kimi_call_song_v2 "$title" "$artist" "$album" "$model" "$api_key"
end
